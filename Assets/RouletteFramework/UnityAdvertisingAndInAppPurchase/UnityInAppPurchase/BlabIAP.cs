...
#if	USE_UNITY_IAP
  public class BlabIAP : MonoBehaviour, IStoreListener
#else
public class BlabIAP : MonoBehaviour
#endif
{

	public InAppConfigScriptableAsset inAppConfigScriptableAsset;
	public GameObject PanelPurchaseResult;
	...
	public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs e)
	{
		...
		for(int x = 0;x < inAppConfigScriptableAsset.consumableProductsList.Length;x++) {
		   if(inAppConfigScriptableAsset.consumableProductsList[x].isEnabled) {

		      switch(e.purchasedProduct.definition.type) {
				case ProductType.Consumable:
					if(e.purchasedProduct.definition.id.Equals(inAppConfigScriptableAsset.consumableProductsList[x].GPlayIOSProductID)) {
						float currentCoinsValue = PlayerPrefs.GetFloat(inAppConfigScriptableAsset.consumableProductsList[x].playerPrefDefValue);
						Debug.Log("[Consumable][InApp Purchase][Got Success Purchase] id : " + e.purchasedProduct.definition.id + " old Money Value : " + currentCoinsValue);
						currentCoinsValue += inAppConfigScriptableAsset.consumableProductsList[x].valueInSimulateDollars;
						PlayerPrefs.SetFloat(inAppConfigScriptableAsset.consumableProductsList[x].playerPrefDefValue,currentCoinsValue);
						Debug.Log("[Consumable][InApp Purchase][Got Success Purchase] id : " + e.purchasedProduct.definition.id + " New Money Value : " + PlayerPrefs.GetFloat(inAppConfigScriptableAsset.consumableProductsList[x].playerPrefDefValue));
					}
				break;
		      }
		   }
		...
		for(int x = 0;x < inAppConfigScriptableAsset.notConsumableProductsList.Length;x++) {
		   if(inAppConfigScriptableAsset.notConsumableProductsList[x].isEnabled) {
				switch(e.purchasedProduct.definition.type) {
				  case ProductType.NonConsumable:
					if(e.purchasedProduct.definition.id.Equals(inAppConfigScriptableAsset.notConsumableProductsList[x].GPlayIOSProductID)) {
						PlayerPrefs.SetInt(inAppConfigScriptableAsset.notConsumableProductsList[x].playerPrefDefValue,1);
						Debug.Log("[NOT Consumable][InApp Purchase][Got Success Purchase] id : " + e.purchasedProduct.definition.id + "  Value : " + "SAVED : " +  PlayerPrefs.GetInt(inAppConfigScriptableAsset.notConsumableProductsList[x].playerPrefDefValue));
					}
				  break;
				}
		   }
	...
	public void Awake()
	{
		...
		for(int x = 0;x < inAppConfigScriptableAsset.consumableProductsList.Length;x++) {
		   ...
		   if(inAppConfigScriptableAsset.consumableProductsList[x].isEnabled) {
				builder.AddProduct(inAppConfigScriptableAsset.consumableProductsList[x].GPlayIOSProductID, ProductType.Consumable, new IDs
				{
					{inAppConfigScriptableAsset.consumableProductsList[x].MacStoreProductID, MacAppStore.Name},
					{inAppConfigScriptableAsset.consumableProductsList[x].TizenProductID, TizenStore.Name},
					{inAppConfigScriptableAsset.consumableProductsList[x].MoolahAppStoreProductID, MoolahAppStore.Name},
				});
		   }
		...
		for(int x = 0;x < inAppConfigScriptableAsset.notConsumableProductsList.Length;x++) {
		   ...
		   if(inAppConfigScriptableAsset.notConsumableProductsList[x].isEnabled) {
				builder.AddProduct(inAppConfigScriptableAsset.notConsumableProductsList[x].GPlayIOSProductID, ProductType.NonConsumable, new IDs
				{
					{inAppConfigScriptableAsset.notConsumableProductsList[x].MacStoreProductID, MacAppStore.Name},
					{inAppConfigScriptableAsset.notConsumableProductsList[x].TizenProductID, TizenStore.Name},
					{inAppConfigScriptableAsset.notConsumableProductsList[x].MoolahAppStoreProductID, MoolahAppStore.Name},
				});
		   }
...
*/
	...
	public void UpdateHistoryUI()
	{
		...
		for (int t = 0; t < m_Controller.products.all.Length; t++)
		{
			...
			itemText += "\n\n" + item.definition.id + " : " +  m_Controller.products.all[t].definition.type;//inAppConfigScriptableAsset.consumableProductsList[t].smallDescription;
